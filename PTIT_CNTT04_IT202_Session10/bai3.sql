...
EXPLAIN ANALYZE
select full_name from users where hometown = 'Hà Nội';

-- Tạo một chỉ mục có tên idx_hometown cho cột hometown của bảng User.
create index  idx_hometown on users(hometown);

-- Chạy lại yêu cầu số (2) với EXPLAIN ANALYZE để kiểm tra kết quả sau khi đánh chỉ mục . So sánh kết quả trước và sau khi đánh chỉ mục.
EXPLAIN ANALYZE
select full_name from users where hometown = 'Hà Nội';
-- so sanh 
-- truoc : quyet toan bo bang xong roi moi tim nhung hometown = "Ha noi"
-- sau : chio quyet nhung chi muc co hometown = "Ha noi"
